...
export class LancamentoService {
  ...
  listarTodosLancamentos(): Observable<any>{
    return this.http
      .get(env.baseApiUrl + this.PATH
        + this.PATH_TODOS_LANC.replace('{funcionarioId}', this.httpUtil.obterIdUsuario()), this.httpUtil.headers());
  }

  listarLancamentosPorFuncionario(funcionarioId: string, pagina: string, ordem: string, direcao: string): Observable<any>{
    let url : string = env.baseApiUrl + this.PATH + this.PATH_LANCAMENTOS.replace('{funcionarioId}', funcionarioId);
  ...
  buscarPorId(lancamnetoId: string):Observable<any>{
    return this.http.get(env.baseApiUrl + this.PATH + '/' + lancamnetoId, this.httpUtil.headers());
  }

  atualizar(lancamento: Lancamento): Observable<any>{
    return this.http.put(env.baseApiUrl + this.PATH + '/' + lancamento.id, lancamento, this.httpUtil.headers());
  }
